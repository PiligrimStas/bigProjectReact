...
// ранее код возвращаемый этой функцие находился в buildLoaders.ts, но мы его вынесли для того чтобы
// функция возвращая этот код могла вызываться ещё и файле config/storybook/webpack.config.ts
// то есть сейчас эта функция будет вызываться не только в buildLoaders.ts но и config/storybook/webpack.config.ts
